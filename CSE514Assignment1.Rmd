---
title: "CSE514 Programming Assignment 1"
...
knitr::opts_chunk$set(echo = FALSE, out.width="70%", out.height="70%" )
```
...
### Description/Formulation of the Problem
  ...
  The topic of this Programming Assignment is to Design and implement a gradient descent algorithm or algorithms for regression. I will use gradient descent to perform linear regression and fit linear models for the given dataset. 
  
  The dataset for this Assignment is the Concrete Compressive Strength dataset in the UCI repository(https://archive.ics.uci.edu/ml/datasets/Concrete+Compressive+Strength). 
  
  There are 1030 instances, 9 attributes, no missing values in this dataset. The first 8 columns are 8 features, the last column is the response variable: concrete strength. I will use 900 instances for training and 130 instances for testing, randomly selected. After pre-processing the data, I'll fit Uni-variate linear regression models and multivariate linear regression models, and compare them to get results and conclusions.
  
  The goal of this assignment is analyze and understand how the concrete strength is impacted by 8 features. The results and conclusions could be worked on improving the concrete strength in civil engineering area.
  
 


### Description of normalizing/standardizing data

  The data contains 8 features, and their scales are vary, which may influence the results of linear regression. According to the plot of the raw data, we can see there's some outliers. So I decide to standardize the data as a pre-processing step (i.e., transform the data to have mean zero and standard deviation one).
  
  To standardize each variable, I will use the formula $X_{standardized} = (X - mean(X)) / std(X))$ and write a function to convert the values of the data into z-scroe values.
  
  The plots of the data before and after standardization:
...
boxplot(new_data, 
        names=c('x1','x2','x3','x4','x5','x6','x7','x8','y'),
        main = "The Distribution of Feature Values after Processing")
```
  

  From two graphs, we can see that after standardization, the values were rescaled. The range of feature values became similar. Also all the features were transformed to have a mean 0 and standard deviation 1. Standardized data is much better to be used for analysis.


### Details of Algorithm

First of all, I randomly selected 900 instances for training and 130 instances for testing on the raw data, saved as `raw`_train` and `raw_test`. Then I did the same step on the pre-processed data.
...
For uni-variate linear regression, the formula $f(x)=mx+b$ is used for every feature, the selected training data would be used for calculation. Also using MSE as the loss function, which is not a stochastic gradient descent. 

To apply the gradient descent algorithm:

Initializing the coefficient and bias as 0. Setting the max iteration number as 1000 to use for stopping criterion, repeat until the max number is reached. 

To choose the learning rate, I started from a really small number, then tried different number for each feature to see which number perform the best, and decide the learning rate.

Then I calculated the MSE loss function. And I calculated the gradients of the cost function with respect to the parameters through the trained data points, and update the coefficient and bias parameters.

After that. I calculate MSE and Variance for model on trained and tested data, then use those values to get variance explained on trained and tested data.



#### Multivariate linear regression

For uni-variate linear regression, I did the similar steps as for uni-variate linear regression. In this case, the formula $f(x)=(m \cdot  x)$ is using, where $m = (m_0, m_1, …, m_p)^T$, and $x = (1, x_1, …, x_p)^T$, with $m_0$ in place of b in the model. Similarly, the selected training data would be used for calculation. Also using MSE as the loss function, which is not a stochastic gradient descent. 

To apply the gradient descent algorithm:

Which is different from the uni-variate linear regression, I'll set predictors data value input in matrix form as $m$ in the matrix form.

First, I assigned a parameter $m_i$ to each of the features, and initialized those parameters and a b parameter as 0. Setting the max iteration number as 1000 to use for stopping criterion, repeat until the max number is reached. 

To choose the learning rate, I started from a small number, then tried different number. Present the result from the function to see which learning rate number perform the best, and I decide the learning rate.

Then I calculated the MSE loss function. And I calculated the multiple gradients of the cost function through the trained data points, and update the parameters.
...
####  Univariate linear regression
    ...
    CALCULATE predicted_y_train, predicted_y_test
    CALCULATE MSE_train, MSE_test
    CALCULATE Variance_train, Variance_test
    CALCULATE VE_train, VE_test

    RETURN Coefficients, VE_train, VE_test, Predictions
...
• The same models that accurately predicted the training data do not accurately predict the testing data. From the Variance Explained table above, the uni-variate linear model of feature `Cement` has $VE=0.18745207$ on the raw training data, but $VE = 0.02018686$ on the raw testing data. The model predicting the training is much better than predicting the testing data.

• Different models do take longer to train or require different  hyperparameter values. The learning rates of different models are different: the step size used on models are different. Also the data given may not be the same. In this assignment, basically, the models using pre-processed data have larger learning rates than those using raw data. 

• Pre-processing changed the scale and distribution of the data. It made the scales similar among different features, and dealt with those outliers. According to the Variance Explained table above, the model using pre-processed data perform better than those using the raw data.

### Conclusion

According to the table provided above:

`Cement` has the highest positive R-squared values. So it is the most important factor to predict concrete compressive strength.

`Water`, `Superplasticizer` and `Age` also have some high R-sqaured values on uni-variate models, so may also be important for predicting.

`Fine Aggregate` and `Coarse Aggregate` are less important.

`Blast Furnace Slag` and `Fly Ash` got negative or quite small values, it shows that they are not important factors for predicting.

To make the hardest possible concrete, changing the value of `Cement`. `Water`, `Superplasticizer` and `Age` would be helpful. According to the correlations, I would suggest increasing the value of `Cement`, and may consider about increasing `Superplasticizer` and 'Age', decreasing `Water` as well.